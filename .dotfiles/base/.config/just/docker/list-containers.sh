#!/usr/bin/env bash
# List containers across Docker and Podman rootful/rootless stores.
...
readonly TABLE_FMT='table {{.Names}}\t{{.Image}}\t{{.Status}}\t{{.ID}}'
...
# Fetch containers. Sets FETCH_OUTPUT and FETCH_COUNT.
...
fetch_containers() {
  ...
  if [[ ${cmd[0]} == "sudo" ]] && [[ -t 0 ]]; then
    ...
    FETCH_OUTPUT=$("${cmd[@]}" ps -a --format "$TABLE_FMT") || return 1
  else
    FETCH_OUTPUT=$("${cmd[@]}" ps -a --format "$TABLE_FMT" 2>/dev/null) || return 1
  fi
...
render_store() {
  ...
  if ! fetch_containers "${STORE_CMD[@]}"; then
